    ...
    <div className="px-4 py-4 md:px-8 md:py-8 space-y-5">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-emerald-600 to-teal-700 p-5 rounded-xl shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs text-emerald-200 font-medium">Total Income</p>
              <p className="text-2xl font-bold text-white mt-1">{currency} {total.toFixed(2)}</p>
            </div>
            <div className="p-2.5 bg-white/20 rounded-xl"><TrendingUp className="text-white" size={20} /></div>
          </div>
        </div>
        ...
        <div className="bg-[#141927] p-5 rounded-xl border border-slate-800">
          ...
          <div className="space-y-2">
            ...
            {topSources.length === 0 && <p className="text-xs text-slate-600">No data yet</p>}
          </div>
        </div>
      </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2 bg-slate-800 border border-slate-700 text-slate-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm"
        />
      </div>
        ...
        <div className="hidden md:block">
          <table className="w-full">
            <thead className="bg-slate-800/40 border-b border-slate-800">
              <tr>
                {['Date', 'Title', 'Account', 'Source', 'Amount'].map((h) => (
            ...
            <tbody className="divide-y divide-slate-800">
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={5} className="px-5 py-10 text-center text-slate-600 text-sm">No income found</td></tr>
              )}
            </tbody>
          </table>
        </div>
        <div className="md:hidden divide-y divide-slate-800">
          {filtered.map((t) => (
            <div key={t.id} className="px-4 py-3 flex items-center justify-between gap-3">
              ...
              <span className="text-sm font-semibold text-emerald-400 shrink-0">+{currency} {t.amount.toFixed(2)}</span>
            </div>
          ))}
          {filtered.length === 0 && (
            <div className="py-10 text-center text-slate-600 text-sm">No income found</div>
          )}
        </div>
      </div>
    </div>
